...
func (r *Dao) NewVideoRecord(ctx context.Context, data *models.VideoInfo) error {
	tx := r.pgdb.Begin()
...
func (r *Dao) GetVideoInfo(ctx context.Context, rvid int64) (*models.VideoInfo, error) {
	tx := r.pgdb.Begin()
	key := utils.GenVideoListKey()
...
func (r *Dao) GetVideoList(ctx context.Context, page int32, pageSize int32) ([]*models.VideoInfo, int64, error) {
	tx := r.pgdb.Begin()
	key := utils.GenVideoListKey()
	...
	if !r.isSyncRunning.Load() {
		go func(data []*models.VideoInfo, r *Dao) {
...
func (r *Dao) GetUserVideoList(ctx context.Context, page int32, pageSize int32, uid int64) ([]*models.VideoInfo, int64, error) {
	tx := r.pgdb.Begin()
	...
	// 异步更新
	if !signal.Load() {
		go func(data []*models.VideoInfo, r *Dao, signal *atomic.Bool) {
			signal.Store(true)
			ctx := context.Background()
			for _, v := range data {
				_ = r.newField(ctx, key, strconv.FormatInt(v.RVID, 10), v)
...
func (r *Dao) GetJudgingVideoList(ctx context.Context, page int32, pageSize int32) ([]*models.VideoInfo, int64, error) {
	tx := r.pgdb.Begin()
