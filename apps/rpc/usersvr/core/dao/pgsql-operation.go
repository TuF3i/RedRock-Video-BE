...
func (r *Dao) ifRecordExist(tx *gorm.DB, uid int64) (bool, error) {
	var count int64
	err := tx.Model(&models.RvUser{}).Where("github_uid = ?", uid).Count(&count).Error
	if err != nil {
		return false, err
	}
...
func (r *Dao) getRecordDetail(tx *gorm.DB, uid int64) (*models.RvUser, error) {
	data := new(models.RvUser)
...
func (r *Dao) newRecord(tx *gorm.DB, data *models.RvUser) error {
...
func (r *Dao) getRecordDetails(tx *gorm.DB, page int32, pageSize int32, role string) ([]*models.RvUser, int64, error) {
	var dataSet []*models.RvUser
	var total int64

	offset := (page - 1) * pageSize
	err := tx.Model(&models.RvUser{}).Count(&total).Error
	if err != nil {
		return nil, 0, err
	}
...
func (r *Dao) setColumnValue(tx *gorm.DB, uid int64, column string, value interface{}) error {
	err := tx.Model(&models.RvUser{}).Where("github_uid = ?", uid).Update(column, value).Error
	if err != nil {
		return err
	}
