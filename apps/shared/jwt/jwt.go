...
func GenerateAccessToken(uid int64, role string) (accessToken string, err error) {
	now := time.Now()
	// AccessToken
	accessClaims := models.MainClaims{
		Uid:  uid,
		Role: role,
...
func GenerateRefreshToken(uid int64, role string) (refreshToken string, err error) {
	now := time.Now()
	// RefreshToken
	refreshClaims := models.MainClaims{
		Uid:  uid,
		Role: role,
...
func VerifyAccessToken(tokenStr string) (*models.MainClaims, error) {
	if tokenStr == "" {
	...
	// 类型断言
	claims, ok := token.Claims.(*models.MainClaims)
	if !ok {
...
func VerifyRefreshToken(tokenStr string) (*models.MainClaims, error) {
	if tokenStr == "" {
		return nil, errors.New("token is empty")
	}
	// 解析JWT token
