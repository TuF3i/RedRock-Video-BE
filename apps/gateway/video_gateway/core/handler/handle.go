...
func UploadVideoHandleFunc() app.HandlerFunc {
	return func(ctx context.Context, c *app.RequestContext) {
		...
		if RawRvid == "" {
			c.JSON(consts.StatusOK, dto.GenFinalResponse[response2.Response](response2.EmptyRVID))
			return
		}
		// 解码rvid
		...
		c.JSON(consts.StatusOK, dto.GenFinalResponse[response2.Response](response2.OperationSuccess))
	}
}
...
func AddVideoHandleFunc() app.HandlerFunc {
	return func(ctx context.Context, c *app.RequestContext) {
		...
		if err != nil {
			resp := dto.GenFinalResponse(response2.ValidateRequestFail)
			c.JSON(consts.StatusOK, resp)
			return
		}
		// 构造请求
...
func JudgeAccessHandleFunc() app.HandlerFunc {
	return func(ctx context.Context, c *app.RequestContext) {
		...
		if claim.Role != union_var.JWT_ROLE_ADMIN {
			c.JSON(consts.StatusOK, dto.GenFinalResponse[response2.Response](response2.YouDoNotHaveAccess))
			return
		}
		// 生成请求
...
func GetPreSignedUrlHandleFunc() app.HandlerFunc {
	return func(ctx context.Context, c *app.RequestContext) {
		...
		// 类型断言
		claim := claims.(*models2.MainClaims)
		// 生成请求
...
func GetNewRvidHandleFunc() app.HandlerFunc {
	return func(ctx context.Context, c *app.RequestContext) {
		rvid := uuid.New().ID()
		resp := dto.GenFinalResponse(response2.FinalResponse{
			Status: 10200,
