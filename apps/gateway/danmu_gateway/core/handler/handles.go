...
func PubVideoDanmuHandleFunc() app.HandlerFunc {
	return func(ctx context.Context, c *app.RequestContext) {
		...
		if !ok {
			c.JSON(consts.StatusOK, dto.GenFinalResponse(response2.YouDoNotHaveAccess))
			return
		}
		// 类型断言
		...
		if err != nil {
			c.JSON(consts.StatusOK, dto.GenFinalResponse(response2.ValidateRequestFail))
			return
		}
		// 填充danmuData
...
func GetHotDanmuHandleFunc() app.HandlerFunc {
	return func(ctx context.Context, c *app.RequestContext) {
		...
		rvid := c.Param("rvid")
		if rvid == "" {
			c.JSON(consts.StatusOK, dto.GenFinalResponse(response2.EmptyRVID))
			return
		}
		// 将string转为int64
